...
static void pedometer_afe_config(void){
     ...
     uint8_t Buf[5]={0x06,0x20,0x00,0x01,0x80}; // 4g mode FS=10    4g--8196
...
static void pedometer_Read(uint8_t *buffer, uint8_t len){
    dvMMA9553_Read(MMA9553_Slave_Addr, MMA9553_Sub_Addr, buffer, len);
}


static void pedometer_Write(uint8_t *buffer, uint8_t len){
...
// static void pedometer_mainrawxyz(void){
//     uint8_t Buf[20];
//     int x, y, z; 
