...
void AFE_Enable_Read ()         //Prohibit writing to registers
{
  ...
  configData[1]=0x00;
  configData[2]=0x01;
  I2c_WriteRegister (AFE_ADDR, AFE_CONTROL0, configData, 3);
}
...
void AFE_Reg_Write (int reg_address, unsigned long data)
{
  ...
  I2c_WriteRegister(AFE_ADDR, reg_address, configData, 3);
}
...
signed long AFE_Reg_Read(int reg_address)
{
    uint8_t configData[3];
  signed long retVal;
  I2c_ReadRegister (AFE_ADDR, reg_address, configData, 3);
  retVal = configData[0];
...
long get_GR_data(){
  return hr3_get_GR_data();
}

long get_RED_data(){
  return hr3_get_RED_data();
}
