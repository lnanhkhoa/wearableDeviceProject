...
bool notifyArrayBLETransmitter;
...
extern uint8 simpleProfileChar6[SIMPLEPROFILE_CHAR6_LEN];
...
void HR_calc(){
    ...
    if(countBleElement >= 20){
        ...
        needToUpdateArrayNotify(); //Update in Task_1
    }
...
void needToUpdateArrayNotify(){
    notifyArrayBLETransmitter = true;
}

void Rewrite_arrayBLETransmitter(){
...
void HR_setHeartRateNumber(){
    if(heartRateNumber> 40 && heartRateNumber < 200){
        memcpy(simpleProfileChar5+1, heartRateNumber, 2);
    }    
}
...
void HR_taskFxn(){
    ...
    measSem = Semaphore_handle(&semStruct);
    
    while(1){
        Semaphore_pend(measSem, BIOS_WAIT_FOREVER);
        HR_measurementTask();
    }
}
