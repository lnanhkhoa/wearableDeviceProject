...
static void clock_main(){
  if(updateClock){
    updateClock = false;
    // sec_t = Clock_getSecond();
    // hour_t = Clock_getHour();
    // min_t = Clock_getMinute();
    WDsDisplay__clearDisplay();
    WDsDisplay__Clock_set(hour, minute);
...
void wearableDevice_updateConfigProfile(uint8_t *newconfigprofile){
